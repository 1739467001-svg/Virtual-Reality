...
- ✋ **移动家具** — 开启“移动模式”，准星对准家具会**高亮**（黄=可拾取、绿=手持），点击拾起后走近台面**自动吸附**摆放，与其它家具/墙**重叠时变红**提醒。**各房间的家具（床 / 床头柜 / 衣柜 / 马桶 / 洗手台 / 淋浴房等）都是可拾取的实体**，能移动 / 旋转 / 调高度 / 换色 / 缩放 / 删除，并随分享链接保存。
...
- 🎵 **背景音乐** — 用 Web Audio API **实时合成**的轻柔环境氛围（缓慢和弦垫 + 起伏 + 空间延迟，无音频文件、无版权问题）；进入房间自动响起，左上 🔊 可静音（记忆偏好）。
...
- 📱 **多端自适应** — 手机 / 平板 / 电脑都适配：左下虚拟摇杆移动、右半屏滑动看四周。**桌面/平板**控制面板停靠右上、分区可折叠（点标题展开/收起）；**手机**则改为 App 式**底部抽屉**：右下「☰ 菜单」唤出，顶部标签快速切换分区，点遮罩 / 把手收起，平时不挡画面也不挡摇杆。
